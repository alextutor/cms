<?php
	...
	$sql="SELECT * FROM elementos_menu ".$criterio; 
	$res=mysql_query($sql); 
  ...
  if(!isset($orden)) 
	...
	$sql="SELECT * FROM elementos_menu ".$criterio." ORDER BY ".$orden." ASC ";
	$columnas[] = 'titulo';
...
<div id="toolbar-box">
	<div class="m">
    	<div class="toolbar-list" id="toolbar">
            ...
            <a href="/webadmin/index.php?option=com_menus_elemen_new" onclick="" class="toolbar"><span class="icon-32-new"></span>Nuevo</a>
            </li>
          ...
          <a href="/webadmin/index.php?option=pantallaprincipal" onclick="Joomla.submitbutton('article.cancel')" class="toolbar"><span class="icon-32-cancel"></span>Cancelar</a>
            ...
            <a  onclick="if (document.adminForm.boxchecked.value==0){alert('Por favor, primero haga una selección desde la lista');}else{
            document.forms['adminForm'].task.value='com_menus_eliminar',
         ...
         <div class="pagetitle icon-48-categories icon-48-content-categories"><h2>Gestor de menús: Elementos del menú</h2></div>            
    </div>
</div>
...
while($registro=mysql_fetch_array($res)) {
 ...
 ?> 
	<tr class="row0">	
    <td class="center">
     <input type="checkbox" id="cb0" name="cid[]" value="<?php echo $registro['idmenu'];?>" onclick="Joomla.isChecked(this.checked);" title="Casilla de selección para la fila 1">										
     </td>                      
 <?php 	
	//$x=0;			   
	//foreach($columnas as $CampoProducto){
