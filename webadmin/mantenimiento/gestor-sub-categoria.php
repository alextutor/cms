<?php
	...
	$criterio = " where sub.idcategoria=cate.idcategoria and sub.estado=1";   
	$sql="SELECT * FROM subcategoria ".$criterio; 
	$sqlnrow="SELECT * FROM subcategoria "; 
	$res=mysql_query($sqlnrow); 
  ...
  if(!isset($orden)) 
	...
	$sql="SELECT sub.idsubcategoria, sub.titulo,sub.estado,sub.descripcion,sub.imagen,sub.orden,cate.titulo as categoria FROM subcategoria  as sub,categoria as cate 
	 " .$criterio ." ORDER BY ".$orden." ASC "; 
	$columnas[] = 'titulo';
	$columnas[] = 'categoria';
	$columnas[] = 'estado';
...
<div id="toolbar-box">
	<div class="m">
    	<div class="toolbar-list" id="toolbar">
            ...
            <a href="/webadmin/index.php?option=com_sub_categories_new" onclick="" class="toolbar"><span class="icon-32-new"></span>Nuevo</a>
            </li>
            ...
            document.forms['adminForm'].task.value='com_sub_categories_editar',
            document.forms['adminForm'].submit() }" class="toolbar"><span class="icon-32-edit"></span>Editar</a></li>
            <li class="divider"></li>
            
              <li class="button" id="toolbar-trash">
                <a  onclick="if (document.adminForm.boxchecked.value==0){alert('Por favor, primero haga una selección desde la lista');}else{
                document.forms['adminForm'].task.value='com_sub_categories_eliminar',
         ...
         <div class="pagetitle icon-48-sub-categories">
           <h2>Gestor de Sub Categorías: Artículos</h2></div>            
    </div>
</div>
...
while($registro=mysql_fetch_array($res)) {
 ...
 ?> 
	<tr class="row0">	
    <td class="center">
     <input type="checkbox" id="cb0" name="cid[]" value="<?php echo $registro['idsubcategoria'];?>" onclick="Joomla.isChecked(this.checked);" title="Casilla de selección para la fila 1">										
     </td>                      
 <?php 	
	//$x=0;			   
	//foreach($columnas as $CampoProducto){
