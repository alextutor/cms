...
<body>	
<?php $sql_contenido = db_query("SELECT c.*,d.cdetcontenido FROM contenido c, contenidodetalle d WHERE c.ccodcontenido=d.ccodcontenido and c.ccodcontenido = '" . $_GET['id'] . "'");
...
<!------------- Fin para resumen -------------->       
					...
					<input type="text" name="orden" id="orden"  maxlength="150" class="box500" value='<?=$row_contenido['cordcontenido']?>'>                        
            ...
            <select id="estado" name="estado" class="inputbox" size="1" aria-invalid="false">
                ...
                <option value="-2" <?php if( $row_contenido['estado']=="-2") echo " selected='selected'"  ?>>Movido a la papelera</option>                     
             </select>
    	                </li>
       ...
       <?php 
	   if($row_contenido['idexcel']<>""){	
		   $sql_obtener = db_query("select * from excel_archivos where idexcel='".$row_contenido['idexcel']."'");
		   $rs_contenidocss = db_fetch_array($sql_obtener);	
		   $miarchivocss=$rs_contenidocss['archivocss'];
		   //exit;
		   //echo $miarchivocss;exit;	   
		}	   
	   ?>             
      <script>
  ...
  <div id="bobcontent2" class="switchgroup1" style="padding-top: 0px; border-top-style: none; padding-bottom: 0px; border-bottom-style: none; overflow: hidden; height: auto;" >            
               <fieldset class="panelform">
                ...
                <option value="70" <?php if( $row_contenido['tamano_cimgcontenido']=="70") echo " selected='selected'"  ?>>70</option>
                       ...
                       <li><label id="duracion_lbl" for="duracion" class="hasTip" title="">Duración</label>	
                        ...
                        <li>
                       	<div style="clear:both;">
	                          ...
	                          <input name="inicioclases" type="date"  value="<?=$row_contenido['inicioclases']?>" 
                               class="campofecha" size="12" id="inicioclases">
                        </div>    
                       </li>
