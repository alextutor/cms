<?php ob_start();	
	...
	$sql = "SELECT * from condominio  WHERE ".$cfiltro ; 
	//echo $sql;exit;
  ...
  if(!isset($orden)) 
	...
	$columnas[] = 'imagenfoto';	
	$columnas[] = 'Total Residentes';	
	$columnas[] = 'estado';
			
	$nAncho[0] = '10%;'; // nombrecondominio
	$nAncho[1] = '10%';	//imagenfoto
	$nAncho[2] = '10%';//estado
...
?> 
	<tr class="row0">	
       ...
       <input type="checkbox" id="cb0" name="cid[]" value="<?=$registro['idcondominio'];?>
...
title="Casilla de selección para la fila 1">	
         ...
         <td width="<?=$nAncho[0]?>">
             <a href='/webadmin/index.php?option=com_articulos_editar&id=<?=$registro['idcondominio']?>&selectseccion=<?=$selectseccion?>&cfiltro=<?=$cfiltro?>'>
          ...
          <td width="<?=$nAncho[1]?>">         
		 <?php			
			 echo "<img src='/timthumb.php?src=" .$registro['imagenfoto']. "&h=80&w=80&zc=0&q=100&s=1' border=0 class='mgredondearesq'/>" ; 
		  ?>
          </td>                             
          <td width="<?=$nAncho[1]?>"><?=$registro['totalresidentes']?></td>
         <td width="<?=$nAncho[1]?>"><?=$registro['estado']?></td>                                               
	</tr>   
<?php 	
	}//fin while 
?>              
