...
,p.telefono1 ,p.telefono2 , dp.nombredepartamento ,mpv.estado FROM condominio c INNER JOIN maestro_propietarios_vivienda mpv ON
 (c.idcondominio = mpv.idcondominio) INNER JOIN propietarios p ON
  (p.codpropietario = mpv.codpropietario) INNER JOIN  condominio_departamentos dp
  ON (dp.iddepartamento = mpv.iddepartamento)
";
...
$sql = "SELECT
    ...
    , `condominio_propietarios`.`telefono1`
    , `condominio_propietarios`.`telefono2`
    , `condominio_departamentos`.`nombredepartamento`
    , `condominio_propietarios_vivienda`.`estado`
    , `condominio_secciones`.`nombreseccion`
    , `condominio_propietarios_vivienda`.`idmaestro`
FROM
    `condominio_propietarios`
    INNER JOIN `condominio_propietarios_vivienda` 
        ON (`condominio_propietarios`.`codpropietario` = `condominio_propietarios_vivienda`.`codpropietario`)
    INNER JOIN `condominio_departamentos` 
        ON (`condominio_departamentos`.`iddepartamento` = `condominio_propietarios_vivienda`.`iddepartamento`)
    INNER JOIN `condominio` 
        ON (`condominio`.`idcondominio` = `condominio_departamentos`.`idcondominio`)
    INNER JOIN `condominio_secciones` 
        ON (`condominio_secciones`.`idseccion` = `condominio_departamentos`.`idseccion`)";
		
		
	$res=mysql_query($sql); 
	$numeroRegistros=mysql_num_rows($res);
	//echo $sql ."----" . $numeroRegistros;
...
title="Casilla de selección para la fila 1">	
         ...
         <td width="<?=$nAncho[2]?>"><?=$registro['nombreseccion']?></td>                                                    
         <td width="<?=$nAncho[3]?>"><?=$registro['telefono1']?></td>   
         <td width="<?=$nAncho[4]?>"><?=$registro['telefono2']?></td> 
         <td width="<?=$nAncho[4]?>"><?=$registro['estado']?></td>                                            
	</tr>   
<?php 	
	}//fin while 
?>              
