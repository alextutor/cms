<?php ob_start();	
	...
	if($_SESSION['nivel']<>'ADMINISTRADOR'){
		$cfiltro .=	" condominio_departamentos.estado<>-2 ";
	}	
	...
	//echo $cfiltro ;exit;
	// ver en carpeta vista  el archivo consul_gestor_departamento_listado.queryxml		
	$sql = "SELECT
    condominio.nombrecondominio
    ,condominio_departamentos.nombredepartamento
    , condominio_departamentos.estado
    , condominio_departamentos.asignado
  ...
  if(!isset($orden)) 
	...
	$columnas[] = 'nombredepartamento';	
	$columnas[] = 'nombreseccion';	
	$columnas[] = 'estado';
	$columnas[] = 'asignado';
		
	$nAncho[0] = '10%;'; // nombrecondominio
	$nAncho[1] = '10%';	//nombredepartamento
	$nAncho[2] = '10%';//nombreseccion
	$nAncho[3] = '10%';//estado	
	$nAncho[4] = '10%';//asignado	
...
?> 
	<tr class="row0">	
       ...
       <input type="checkbox" id="cb0" name="cid[]" value="<?=$registro['iddepartamento'];?>
...
title="Casilla de selección para la fila 1">	
         ...
         <td width="<?=$nAncho[0]?>">
             <a href='/webadmin/index.php?option=com_articulos_editar&id=<?=$registro['iddepartamento']?>&selectseccion=<?=$selectseccion?>&cfiltro=<?=$cfiltro?>'>
         ...
         <td width="<?=$nAncho[1]?>"><?=$registro['nombredepartamento']?></td>  
          <td width="<?=$nAncho[1]?>"><?=$registro['nombreseccion']?></td>       
         <td width="<?=$nAncho[2]?>"><?=$registro['estado']?></td>                                               
         <td width="<?=$nAncho[3]?>"><?=$registro['asignado']?></td>   
	</tr>   
<?php 	
	}//fin while 
?>              
