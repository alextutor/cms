<?Php  session_start(); 
	extract($_POST);
	$selectseccion=$_GET['selectseccion']; 
	 include($_SERVER['DOCUMENT_ROOT']. '/config.php');	
	$_SESSION['id']=$_GET["id"];
	$modulo="1400"; // galeria de fotos tabla webmodulos
...
<div id="toolbar-box">
	<div class="m">
        <div class="toolbar-list" id="toolbar">
          ...
          <a href="/webadmin/index.php?option=com_fotos_album" onclick="Joomla.submitbutton('article.cancel')" class="toolbar"><span class="icon-32-cancel"></span>Cancelar</a>
            </li>            
            </ul>
         </div>
         <div class="pagetitle icon-48-category-add icon-48-content-category-add">
           <h2>Insertar Fotos Album: Añadir una nueva Foto</h2></div></div><div id="element-box">
		<div class="m">
        	<!--Inicio Formulario -->
          
<form id="current_form" name="current_form"   method="post" action="/include/plupload/upload_album.php">
	<ul class="adminformlist">	
        ...
        <select name="selectpage" id="selectpage" style="width:320px">
                     ...
                     <li><label id="jform_title-lbl" for="jform_title" class="hasTip" title="">Album <span class="star">&nbsp;*</pan></label>                                
                      ...
                      <SELECT name="selectseccion" id="selectseccion">
                        <?php 
                        $clavebuscadah=mysql_query("select * from seccion where ccodseccion='".$selectseccion."'") ;
                        while($row = mysql_fetch_array($clavebuscadah))
                        {
                        echo'<OPTION VALUE="'.$row['ccodseccion'].'">'.$row['cnomseccion'].'</OPTION>';
                        }                         
                        ?>
                      </SELECT>
                      </li><br />                      
           <li><label id="jform_published-lbl" for="jform_published" class="hasTip" title="" aria-invalid="false"></label>	           
            <div id="uploader">
