<?php session_start(); 	
	...
	extract($_POST);
	$selectseccion=$idseccion;

	  // Inicio si el usuario no es administrador esconde los registros borrados	
	if($_SESSION['nivel']=='ADMINISTRADOR'){$cfiltronivel="";}
	else{$cfiltronivel=" c.estado<>-2 and ";}	
	
	if ($selectseccion=="escojaopcion") {$selectseccion="";}
			
	if ($selectseccion<>"") {
		$sql = "SELECT  c.ccodcontenido, c.ccodcategoria,c.dfeccontenido,c.cnomcontenido,c.camicontenido,c.cordcontenido, c.cestcontenido, c.nviscontenido,c.nnrocomentario,c.ctipcontenido,c.cimgcontenido,c.estado FROM contenido c  left join seccioncontenido  s on c.ccodcontenido=s.ccodcontenido WHERE ".$cfiltronivel ." c.ccodpage= '".$pagew."'   " . $row_query . " and (c.ccodmodulo='1100' or c.ccodmodulo='1200') and s.ccodseccion='".$selectseccion."'  group by c.ccodcontenido desc  order by c.dfeccontenido desc";
	}	
	if ($cfiltro<>"") {
		$sql = "SELECT  c.ccodcontenido, c.ccodcategoria,c.dfeccontenido,c.cnomcontenido,c.camicontenido,c.cordcontenido, c.cestcontenido, c.nviscontenido,c.nnrocomentario,c.ctipcontenido,c.cimgcontenido,c.estado FROM contenido c  left join seccioncontenido  s on c.ccodcontenido=s.ccodcontenido WHERE ".$cfiltronivel ." c.ccodpage= '".$pagew."'   " . $row_query . " and (c.ccodmodulo='1100' or c.ccodmodulo='1200') and c.cnomcontenido like '%".$cfiltro."%'  group by c.ccodcontenido desc  order by c.dfeccontenido desc";
		
	}
	//if ($cfiltro=="" and $selectseccion=="") {
	if ($selectseccion=="") {
		$sql = "SELECT  c.ccodcontenido, c.ccodcategoria,c.dfeccontenido,c.cnomcontenido,c.camicontenido,c.cordcontenido, c.cestcontenido, c.nviscontenido,c.nnrocomentario,c.ctipcontenido,c.cimgcontenido,c.estado FROM contenido c  left join seccioncontenido  s on c.ccodcontenido=s.ccodcontenido WHERE ".$cfiltronivel ." c.ccodpage= '".$pagew."'   " . $row_query . " and (c.ccodmodulo='1100' or c.ccodmodulo='1200')  group by c.ccodcontenido desc  order by c.dfeccontenido desc";
	}		
...
<?php 
	...
	echo $idseccion."huizaaaddddddddddddddddddddddddddddddddddddddddddddddd----".$idseccion;
	//exit;
?>              	
