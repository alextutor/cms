...
//mas chiquito quite and pm.cubimenu='1'
...
<tr class="row0">	
    ...
    <td width="<?=$nAncho[$x]?>"><?=$registro['cnommenu']?></td> 
      <td width="<?=$nAncho[$x]?>"><?=$registro['cordmenu']?></td>                                       
  	  ...
  	  <!------------------------------Inicio While 2 --------------------->
         <?php
	if ($registro['cnommenu']<>"Pie"){	//Inicio  no se muestra subniveles si es pie 		
		...
		$sql2_query = "SELECT s.ccodseccion,s.cnomseccion,s.camiseccion, s.curlseccion,s.ctipseccion,s.cnivseccion,s.cordcontenido FROM  seccion s WHERE s.ccodseccion like '". $codsec . "%' and s.ccodpage='".$codigo."'   and s.cnivseccion<>'1' and s.cnivseccion<>'3' and  s.cestseccion='1'   ORDER BY s.cordcontenido ASC"; 
        ...
        <tr class="row0">	
            ...
            <td width="<?=$nAncho[$x]?>"><?=$espacio2?><a href='/webadmin/index.php?option=com_menus_editar&submenu=SI&id=<?=$row2['ccodseccion'];?>'><span style="color:#F00"><?=$row2['cnomseccion']?></span></a></td>                   
            <td width="<?=$nAncho[$x]?>"><span style="color:#F00"><?=$registro['cnommenu']?></span></td>
            <td width="<?=$nAncho[$x]?>"><span style="color:#F00"><?=$row2['cordcontenido']?></span></td> 
        </tr>
        
        
         <!------------------------------Inicio While 3 --------------------->
         <?php		
		}	 //Fin   no se muestra subniveles si es pie	- deberia ponerlomas abajo  
		
		 //row2['ccodseccion']		 
		 $codsec3= substr($row2['ccodseccion'],0,18);  // Utilizado en while 2 para los sub-secciones 	  
		$sql3_query = "SELECT s.ccodseccion,s.cnomseccion,s.camiseccion, s.curlseccion,s.ctipseccion,s.cnivseccion,s.cordcontenido FROM  seccion s WHERE s.ccodseccion like '".$codsec3. "%' and s.ccodpage='".$codigo."' and s.cnivseccion=3 and  s.cestseccion='1'   ORDER BY s.cordcontenido ASC"; 										
		//echo $sql3_query;exit;
		
		$rsSql3 = db_query($sql3_query);
		while($row3 = db_fetch_array($rsSql3)) ////// Inicio while 3		
			{	
			$linea = $linea + 1;			
			if ($row3['cnivseccion']=='1') $espacio2="";
			if ($row3['cnivseccion']=='2') $espacio2="&nbsp;&nbsp;&nbsp;";
			if ($row3['cnivseccion']=='3') $espacio2="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
			if ($row3['cnivseccion']=='4') $espacio2=		
			"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp";
		?>	
        <tr class="row0">	
            <td class="center">
            ...
            <td width="<?=$nAncho[$x]?>"><?=$espacio2?><a href='/webadmin/index.php?option=com_menus_editar&submenu=SI&id=<?=$row3['ccodseccion'];?>'><span style="color:#2b9040;font-weight:bold"><?=$row3['cnomseccion']?></span></a></td>                   
            <td width="<?=$nAncho[$x]?>"><span style="color:#2b9040"><?=$registro['cnommenu']?></span></td>
            <td width="<?=$nAncho[$x]?>"><span style="color:#2b9040; font-weight:bold"><?=$row3['cordcontenido']?></span></td> 
        </tr>
		<?php
			}///////////////////////////////////////fin while 2
		?>  
