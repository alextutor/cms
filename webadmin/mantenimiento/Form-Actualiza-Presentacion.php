...
include($_SERVER['DOCUMENT_ROOT']. '/config.php'); //$fechahoy= date("d-m-Y");
?>
<?php	
	...
	function procesar_estilos_antigua($form_entrada)
	{
		$mostrarsec = new xajaxResponse('ISO-8859-1');
		$opsec   = "<ul class='stylos'>";
		$sqlsec  = " SELECT * FROM estiloseccion WHERE ccodmodulo='1800' and cestsecestilo='1' order by ccodsecestilo";
	...
	function procesar_estilos($value)
	{		
		//$form_entrada		
		$mostrarsec = new xajaxResponse('ISO-8859-1');
		$opsec   = "<ul class='stylos'>";
		...
		$sqlmsec = mysql_query($sqlsec);
		$na = 1;		 
		  $opsec .=" <select name='selectestilo' id='selectestilo' style='width:340px' onChange='xajax_procesar_estilos(xajax.getFormValues('form'))'>";
		while ($rows = mysql_fetch_array($sqlmsec)) 
		{	
			$opsec .= '<option value='.$rows['ccodsecestilo'].' >'.$rows['cnomsecestilo'].'</option>';
			if ($rows['ccodsecestilo']==$value){
				$imagen ="<img src='/webadmin/estilos/images/" .$rows['cimgsecestilo']."' width='50' height='50'/>";	
			}
	    	$na++;				
		}
   	   $opsec .= " </select> "; 
...
?>
<!-- header.php y estilo.css deverian estar en webadmin/index pero fueron desabilitados de alli porque no funcionaria modulo
y noi cambiaria estilos pero ya no es necesario-->
        ...
        <?php $sql_contenido = db_query("SELECT * FROM pagehome WHERE ccodinicio = '" . $_GET['id'] . "'");
			...
			<div class="width-60 fltlft">
               <fieldset class="adminform">
				<legend>Detalles</legend>
					<ul class="adminformlist">	                    	
                       ...
                       <li><label id="jform_published-lbl" for="jform_published" class="hasTip" title="" aria-invalid="false"><strong>Tipo Contenido</strong></label>	          
                        ...
                        <li><label id="jform_published-lbl" for="jform_published" class="hasTip" title="" aria-invalid="false"><strong>Estado</strong></label>	
                 <select id="cesthome" name="cesthome" class="inputbox" size="1" aria-invalid="false">
                <option value="1" <?php if( $row_contenido['cesthome']=="1") echo " selected='selected'"  ?>>Publicado</option>
                <option value="2" <?php if( $row_contenido['cesthome']=="2") echo " selected='selected'"  ?>>Despublicado</option>
                <option value="3" <?php if( $row_contenido['cesthome']=="3") echo " selected='selected'"  ?>>Archivado</option>
                <option value="-2" <?php if( $row_contenido['cesthome']=="-2") echo " selected='selected'"  ?>>Movido a la papelera</option>                     
                  </select>
   	                  </li>    
                ...
                <option value="si" <?php if( $row_contenido['mostrar_titulo']=="si") echo " selected='selected'"  ?>>Si</option>
                <option value="no" <?php if( $row_contenido['mostrar_titulo']=="no") echo " selected='selected'"  ?>>No</option>               
                  </select>
   	                  </li>    
