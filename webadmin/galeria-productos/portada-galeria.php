...
?>
<?php include_once($_SERVER['DOCUMENT_ROOT'].'/webadmin/include/configuration.php'); 
	  ...
	  while( ($fila=mysql_fetch_array($datos)) ) { //al terminar while el puntero esta en el ultimo registro						
		...
		?>
          <div  class=fondocate>
             ...
             <div class="imagen"> 						
                <!-- <a href="index.php?aside=total&idsec1=/webadmin/galeria-productos/galeria&CateId=<?php echo $fila['idcategoria']?>">						
                 <img src='timthumb.php?src=<?php echo $imagen ?>&h=150&w=140&zc=0&q=100&s=1' border=0 title="<?php echo $fila['titulo']?>" />	   
                </a> -->                            
          		<?php 
				$_SESSION['descrprodu'] =$rsCategoria['titulo'];
				$_SESSION['directorio'] ="/webadmin/galeria-productos/";				 					
				//echo '<a href="puertas-madera/'. "galeria" .'/'. trim($fila["idcategoria"]) .'">';     				
				
				echo '<a href="' . "galeria" .'/'. urls_amigables(trim($fila["titulo"])) .'-'. trim($fila["idcategoria"]) .'">';     				
