...
(function($){
	$.fn.extend({ 
		...
		var options = $.extend(defaults, options);
			...
			// check to make sure inview isnt greater than the number of images. 
			//inview should be at least two less than numimages (otherwise hinting wont work and animating left may catch a flash), but one less can work
			...
			{
				$(obj).append('<div id="progress'+randID+'" class="ic-progress-bar" style="position:absolute;bottom:0;background:#91b815;left:0;z-index:1"></div>');
				$('#progress'+randID).width('100%').height(5).css('opacity','1');
			}
			...
			for(i=1;i<=o.inView;i++)
			{
				...
				$('#textholder'+randID+'_'+i).height(imgHeight*o.textholderHeight).css({'backgroundColor':'#FFF'});
			...
			function showtext(t,i)
			{
				...
				if(t != null)
				{
					...
					$('#textholder'+randID+'_'+i).stop().animate({marginBottom:'0px'},500); // Raise textholder Levante textholder
			...
			if(o.displayThumbnails)
			{
				...
				// Build thumbnail viewer and thumbnail divs Crear visor de miniaturas y divs miniatura
...
//					$('#thumbs'+randID).append('<div class="thumb" id="thumb'+randID+'_'+(i+1)+'" style="cursor:pointer;display:inline;float:left;width:'+o.thumbnailWidth+';height:'+o.thumbnailHeight+';padding:0;overflow:hidden;text-align:center;border:2px solid #ccc;margin-right:4px;font-size:'+o.thumbnailFontSize+';font-family:Arial;color:#000;text-shadow:0 0 3px #fff"><img src="thumbnail.php?i='+thumb+'&w='+o.thumbnailWidth+'&h='+o.thumbnailHeight+'&q=100" alt="" /></div>');
					if(i<=o.inView) $('#thumb'+randID+'_'+i).css({'border-color':'#91b815'});
					unviewable.push(i+1);
				}
...
//				$('#thumb'+randID+'_'+numImages).css({'background-image':'url('+thumb+')'});
				...
				//alex $('#thumbs'+randID+' div.thumb:not(:first)').css({opacity:.1}); // makes all thumbs 65% opaque except the first one

				//alex $('#thumbs'+randID+' div.thumb').hover(function(){$(this).animate({'opacity':1},150)},function(){if(viewable[0]!=this.id.split('_')[1]) $(this).animate({'opacity':.65},250)}); // add hover to thumbs
...
//				if(!o.displayThumbnailNumbers) $('#thumbs'+randID+' div').text('');
			...
			if(o.showControls)
			{
				...
				$(obj).append(html);
				var status = 'play';
				//alex $('#play_pause_btn'+randID).css('opacity',.5).hover(function(){$(this).animate({opacity:'1'},250)},function(){$(this).animate({opacity:'.5'},250)});
				...
				//imagen de los botoens adelane y atras
			...
			function forcePause()
			{
				...
				{
					o.onPauseClick.call(this);
					$('#play_pause_btn'+randID).fadeTo(250,0,function(){$(this).css({'background-position':'0 -16px','opacity':'1'});}).animate({opacity:1},250);
					autopilot = 0;
					showminmax();
					$('#progress'+randID).stop().fadeOut();
					clearTimeout(clearInt);
			...
			function preMove()
			{
				...
				if(o.displayThumbnails) for(i=1;i<=numImages;i++) $('#thumb'+randID+'_'+i).css({'border-color':'#ccc'}).animate({'opacity': 1},500);
			}
			...
			function postMove()
			{
				...
				if(o.displayThumbnails) for(i=0;i<viewable.length;i++) $('#thumb'+randID+'_'+viewable[i]).css({'border-color':'#91b815'}).animate({'opacity': 1},500);
