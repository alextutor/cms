...
$adssql   = db_query("Select p.*,e.cdesclase FROM pagehome p,estiloclases e  where e.ccodclase = p.ccodclase and ccodinicio='".$codbanner."'");
...
if ($rowads['ctiphome']=='4')  //----------Contenido Dinamico  --Aqui entra tambien en Portada como mostrar catalogo productos
{
	...
	else
	{
	 ...
	 $sql_query   = "Select c.ccodcontenido,c.cnomcontenido,c.camicontenido,c.crescontenido,c.curlcontenido,s.ccodseccion,c.cimgcontenido,c.ccodmodulo,c.dfeccontenido,c.precio,c.garantia FROM contenido c, seccioncontenido s where c.ccodcontenido=s.ccodcontenido and  c.cestcontenido='1' and c.ccodmodulo='".$rowads['ccodmodulo']."' ".$sql_seccion.$sql_cate." order by c.ccodcontenido desc ";
...
$reg     = ($pag-1) * $pagsecc;
$hometabla = db_query($sql_query . " LIMIT " .$reg." , ".$pagsecc);
