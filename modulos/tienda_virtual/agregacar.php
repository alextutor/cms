...
include_once ($_SERVER['DOCUMENT_ROOT']. '/rutinas/conexion_gamatell.php') ;   

echo $id;exit;

//mysql_select_db("sisdataw_carrito_compras");
...
$qry=mysql_query("select * from productos where idproductos='".$id."'");
...
$carro[md5($id)]=array('identificador'=>md5($id),'cantidad'=>$cantidad,'producto'=>$row['nombproductos'],'nomproducto'=>$row['nombproductos'],'precio'=>$row['precio_dolar'],'rutaimagen'=>$row['rutaimagen'],'id'=>$id);
...
//header("Location:catalogo.php?".SID); alex
//header("Location:vercarrito.php");
//header ("Location:".$path."/principal.php?pagina=carritodominio");exit; 	
//header ("Location:".$path."/principal.php?pagina=tienda-virtual/confirmarcompra");exit;
header ("Location:".$root."/principal.php?pagina=tienda_virtual/vercarrito");
?>