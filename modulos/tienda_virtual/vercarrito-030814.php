...
require_once ('/webadmin/xajax/xajax.inc.php');
	$xajax = new xajax("vercarrito.php");
...
function procesaForm($form_values) { 
	$respuestaXajax = new xajaxResponse('ISO-8859-1');
	$nombre = $form_values['image'];
	$apellidos = $form_values['image']; 
	$direccion = $form_values['image'];  
...
<style type="text/css">
.tablacarritocompras{width:645px;float:left;margin-left:27px;margin-top:8px;}
...
.conte_btn_compraractualizar{width:645px;float:left;text-align:right;margin-top:10px;}
...
<body>
<div id="formDiv"> 
<form method="post" action="javascript:void(null)" id="formvercarrito">
     ...
     <?php if($carro){ ?>  
        ...
        <table border="0" align="center" class="items_tablacarritocompras">        
          <tbody><tr>
            ...
            <td class="cantidad_tabla"><input name="txt_Cantidad" type="text" value="<?php echo $v['cantidad']?> " maxlength="3" size="2" id="txt_Cantidad" class="TIE_cajaTexto3" onKeyPress="javascript:return validarNro(event)"></td>
            <td class="subtotal_tabla"><?php echo number_format($subto,2); ?></td>
            <td class="borrar_tabla">
            <a href="/modulos/tienda_virtual/borracar.php?<?php echo SID ?>&id=<?php echo $v['id'] ?>">
            <input type="image" name="btn_Eliminar" id="btn_Eliminar" title="Eliminar item" src="/modulos/tienda_virtual/imagen/trash.gif" style="border-width:0px;"></a></td>
          </tr>   
          <?php }?>      
          <tr class="tr_total">
           ...
           <div class="conte_btn_compraractualizar">
            ...
            <a onClick="xajax_procesaForm(xajax.getFormValues('formvercarrito'));" id="" href="">
         <img src="/modulos/tienda_virtual/imagen/btn_actualizar.jpg" width="91" height="23" alt="Actualizar" class="btn_actualizar">
         </a>
        </div>
