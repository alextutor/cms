<?php
//echo $codcont."---sdasdas";exit;
$sqlartidere="SELECT * FROM  contenido c, contenidodetalle  cd WHERE c.ccodcontenido=cd.ccodcontenido and c.ccodcontenido='".$codcont ."'";
$nTotal = mysql_query("SELECT FOUND_ROWS()");   
//echo $sqlartidere;exit;
$sqlcontenido = db_query($sqlartidere);
$nconta=1;
...
<div id="articulo">
    <div class="contenido_articulo">
         <a  rel="shadowbox[galeria1]" href="<?=$row['cimgcontenido']?>"  title="<?=$row['cnomcontenido']?>">
                   ...
                   alt="<?=$row['cnomcontenido']?>"  width="<?=$row['tamano_cimgcontenido']?>%"  class="imagen_arti_derecha"></a>  
...
<!-- articuloderecha-14-05-2016.php esta  la imagen y el texto en un div --> 
