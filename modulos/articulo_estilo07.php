...
<br />
<?php
$sql_query = "SELECT * FROM  contenido c, seccioncontenido s, contenidodetalle d WHERE c.ccodcontenido=s.ccodcontenido and c.ccodcontenido=d.ccodcontenido and s.ccodpage='".$codpage."' and s.ccodseccion = '".$codsecc."' AND c.cestcontenido='1' and c.ccodcategoria='1' order by c.dfeccontenido desc ";
...
?>
		<div id='hometitulo'><?=$row['cnomcontenido']?></div>
		<div id="homecontenido">
               
		<?=$row['cdetcontenido']?> <a href="<?=$enlaceurl?>" title="<?=$row['cnomcontenido']?>" <?=$enlacedestino?>><b>Leer mas &raquo;</b></a>
		</div>
<?php }?>
