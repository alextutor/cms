...
.desc_galeria{margin-bottom: 10px;width: 100%;font-size: 14px;background: darkorange;
    text-align: center;color: white;padding: 5px; font-weight: bold;}
...
<div class="desc_galeria">
	<div><?php echo "Otras " . $rowSecciongaleria['cnomseccion'] ?></div>
<?php //echo $rowSecciongaleria['cdesseccion'] ?>
</div>
<div class="my-gallery" itemscope itemtype="http://schema.org/ImageGallery">
	<?php 
    $sql_query = "SELECT * FROM  contenido c, seccioncontenido s WHERE c.ccodcontenido=s.ccodcontenido  and s.ccodseccion = '".$codsecc."' AND c.cestcontenido='1' AND c.estado='1' and c.ccodcategoria='1' and c.ccodcontenido<>'".$codcont ."' order by c.dfeccontenido desc ";
    ...
    ?>
        <figure itemprop="associatedMedia" itemscope itemtype="http://schema.org/ImageObject">
         <a href="<?=$enlaceurl?>" title="<?=$row['cnomcontenido']?>" <?=$enlacedestino?>>
              ...
              <img src="/timthumb.php?src=<?=$row['cimgcontenido'] ?>&h=100&w=80&zc=0&q=100&s=1" itemprop="thumbnail" alt="<?=$row['cnomcontenido']?>" />
          </a>
