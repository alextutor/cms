...
?>
	<div class="seccionindex50">
		<dl class="seccionindex" >
			...
			{
			?>
			<a href="<?=$enlaceurl?>" title="<?=$row['cnomcontenido']?>" <?=$enlacedestino?>><img src="<?=ereg_replace('/fotos/','/thumbs/',$row['cimgcontenido'])?>" border="0" title="<?=$row['cnomcontenido']?>"  alt="<?=$row['cnomcontenido']?>" ></a>
			<?php } ?>
			</dt>
			<dd>
