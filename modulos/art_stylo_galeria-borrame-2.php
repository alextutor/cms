     ...
     <?php   				
	  ...
	  while( ($rowfila=mysql_fetch_array($rsgaleria)) ) { //al terminar while el puntero esta en el ultimo registro						
		  if ($contador == 0) {  //si 1
			...
			//echo "<div style='margin: 0 auto;'>";  //abre 1	deveria hbilitarse pero si lo hago el pie se biene para arriba investigar	  	 
		  }   //Fin Si 1	
		...
		?>
         ...
         <img src='/timthumb.php?src=<?=$rowfila['cimgcontenido'] ?>&h=135&w=90&zc=0&q=100&s=1' border=0 title="<?=$fila['cnomcontenido']?>" />	 
                 </a>	                                                 			                     											  
              </div>
          </div>
        <?php
