...
while ($rowmizq = db_fetch_array($sqlmizq))
{
	...
	$sqlsubmenu = db_query("SELECT s.ccodseccion,s.cnomseccion,s.camiseccion,s.curlseccion,s.ctipseccion FROM  seccion s, seccionmenu  sm WHERE s.ccodseccion=sm.ccodseccion  and sm.ccodmenu='".$rowmizq['ccodmenu']."' and s.ccodpage='".$codpage."' and s.cnivseccion=1 and  s.cestseccion='1'  ORDER BY sm.cordmenu");
